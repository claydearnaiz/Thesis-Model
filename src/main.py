...
Real-time person detection within ROI zones.

Usage:
    python src/main.py                              # default (yolov8s)
    python src/main.py --model yolov8n              # use YOLOv8n
    python src/main.py --model mobilenet-ssd        # use MobileNet-SSD
    python src/main.py --model yolox-nano           # use YOLOX-Nano
...
def parse_args():
    parser = argparse.ArgumentParser(description="Canteen ROI Monitoring System")
    parser.add_argument("--model", type=str, default="yolov8s",
                        choices=list_models(),
                        help=f"Detection model to use ({', '.join(list_models())})")
    ...
    parser.add_argument("--source", type=int, default=None,
                        help="Camera source index (overrides config)")
...
def setup_logger(enabled: bool):
    ...
    writer.writerow(["timestamp", "roi_name", "person_count"])
...
def main():
    ...
    cam_source = args.source if args.source is not None else config.get("camera_source", 0)
        ...
        while True:
            ...
            if fps_counter == 1:
                ...
                for roi_name, count in roi_counts.items():
                    if count > 0:
                        status_parts.append(f"{roi_name}: {count} person(s)")
                    else:
                        status_parts.append(f"{roi_name}: empty")

                timestamp = datetime.now().strftime("%H:%M:%S")
                status = " | ".join(status_parts) if status_parts else "No ROIs"
                print(f"[{timestamp}] FPS: {display_fps:.1f} | {status}")
            ...
            if not args.no_display:
                ...
                model_label = f"Model: {detector.name} | FPS: {display_fps:.1f} | Persons: {len(detections)}"
                cv2.putText(frame, model_label, (10, 25),
